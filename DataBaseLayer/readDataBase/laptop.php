<?php
  ...
  function singleProduct($id){
    ...
    $s = "SELECT * FROM laptop WHERE productID = $id "; 
    $result = mysqli_query($con,$s);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $name = "Laptop ".$row["brand"]." ".$row["processorType"];
        $model = "Laptop ".$row["productID"];
        $keyFeatures = $row["brand"]." ".$row["processorType"]." Processor<br>".$row["screenSize"]."  Screen ".$row["resulation"]."<br>".$row["GraphicsCardCapacity"]." ".$row["HardDiskCapacity"]." RAM + ".$row["ssdCapacity"]." SSD";
